...
else ifeq ($(TARGET_PRODUCT), $(filter $(TARGET_PRODUCT),WW_Tinker_Edge_R))
	ifeq ($(TARGET_BUILD_VARIANT),user)
		LOCAL_SRC_FILES := config/WW_Tinker_Edge_R/user/$(LOCAL_MODULE)
	else
		LOCAL_SRC_FILES := config/WW_Tinker_Edge_R/debug/$(LOCAL_MODULE)
	endif
else
	ifeq ($(TARGET_BUILD_VARIANT),user)
